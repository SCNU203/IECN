        ...
        for i, inputs in enumerate(data_loader):
            ...
            if self.adjustment == 'feature-wise' or self.adjustment == 'Combined':
                for j in range(self.n_splits):
                    splits_loss = self.pid_criterion(outputs[j], pids)
                    source_pid_loss.append(splits_loss)
                ...
                source_pid_loss.append(self.pid_criterion(outputs, pids))
            ...
            loss_print = {}
            loss_print['s_pid_loss'] = source_pid_loss.sum() / self.n_splits
            loss_print['t_un_loss'] = loss_un.sum() / self.n_splits
