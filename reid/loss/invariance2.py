...
class InvNet(nn.Module):
    def __init__(self, num_features, num_classes, beta=0.05, knn=6, alpha=0.01, n_splits=10):
        ...
        self.ems = nn.ModuleList()
        num_splits = num_features // self.n_splits
        if num_splits * self.n_splits < num_features:
            num_splits = num_splits + 1
        self.num_splits = num_splits
        count = 0
        for i in range(self.n_splits):
            count = count + num_splits
            if count > num_features:
                num_splits = num_features - (count - num_splits)
            self.ems.append(nn.Parameter(torch.zeros(num_classes, num_splits), requires_grad=False))
    ...
    def forward(self, inputs, targets, epoch=None):
        ...
        loss = []
        for i in range(self.n_splits):
            sim = ExemplarMemory.apply(inputs[:, self.num_splits * i: self.num_splits * (i + 1)], targets, self.ems[i], alpha)
            sim = sim / self.beta
            if self.knn > 0 and epoch > 4:
                # With neighborhood invariance
                splits_loss = self.smooth_loss(sim, targets)
                loss.append(splits_loss)
            else:
                # Without neighborhood invariance
                splits_loss = F.cross_entropy(sim, targets)
                loss.append(splits_loss)
